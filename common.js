(function () {
  ...
  function wirePageAnimations() {
    const body = document.body;
    if (!body || body.dataset.animationsWired === "1") {
      return;
    }

    body.dataset.animationsWired = "1";
    ...
    const introSelectors = [
      ...
      ".hero-stats",
      ".hero-actions",
      ".tracker-header .section-kicker",
      ".tracker-title",
      ".tracker-subtitle",
      ".tracker-summary .summary-pill",
    ...
    const introNodes = collectUniqueElements(introSelectors).filter(function (node) {
      return !isVisuallyHidden(node);
    });

    introNodes.forEach(function (node, index) {
      node.classList.add("anim-enter");
      node.style.setProperty("--anim-delay", `${Math.min(index * 80, 560)}ms`);
    });

    const introSet = new Set(introNodes);
    const revealNodes = collectUniqueElements(revealSelectors).filter(function (node) {
      return !introSet.has(node) && !isVisuallyHidden(node);
    });

    revealNodes.forEach(function (node, index) {
      node.classList.add("anim-reveal");
      node.style.setProperty("--anim-delay", `${(index % 6) * 70}ms`);
    });
    ...
    if (prefersReducedMotion) {
      markVisible(introNodes);
      markVisible(revealNodes);
      return;
    }
